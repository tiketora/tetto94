...
export function buildAnalysisEmailHtml(input: CalcoloInput, output: CalcoloOutput, citta?: string): string {
  const rows: Array<[string, string]> = [
    ['Zona climatica', ZONA_LABELS[input.zona]],
    ['Superficie', `${input.superficie} m²`],
    ['Età copertura', ETA_LABELS[input.fasciaEta]],
    ...
    ['Materiale', MATERIALE_LABELS[input.materiale]],
    ['Ultimo intervento', ULTIMO_INTERVENTO_LABELS[input.ultimoIntervento]],
    // Free-typed by the visitor and optional — only added when present, and
    // shown last since it's a lower-confidence field than the quiz answers
    // above (it's raw text, not validated against a fixed set of options).
...
export function buildAnalysisEmailText(input: CalcoloInput, output: CalcoloOutput, citta?: string): string {
  return `NUOVA ANALISI T94 ROOF INDEX™ – TETTO94
