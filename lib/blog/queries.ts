...
/**
 * Public: the N most recently published posts — powers the homepage blog
 * preview section. `getPublishedPosts` is already sorted newest-first, so
 * this is just a bounded slice, never draft/unpublished content.
 */
export async function getLatestPublishedPosts(limit = 4): Promise<BlogPost[]> {
  const posts = await getPublishedPosts()
