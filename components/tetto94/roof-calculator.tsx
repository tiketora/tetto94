...
// import { useState, useCallback } from 'react'
// import { motion, AnimatePresence } from 'framer-motion'
// import { ArrowRight, ArrowLeft, RotateCcw, Phone, MessageCircle, MapPin, Lock, CheckCircle2 } from 'lucide-react'
// import {
//   type CalcoloInput,
//   type CalcoloOutput,
//   type ZonaClimatica,
...
//     id: 8,
//     field: 'telefono',
//     question: 'Un ultimo passo per la tua offerta migliore',
//     tag: '08 — CONTATTO',
//     type: 'contact',
//   },
// ]
...
//           className="flex flex-col gap-6"
//         >
//           {/* Cost estimate / lead confirmation — the phone number was
//               already collected as a required step of the quiz, so there is
//               nothing left to "unlock" here: this panel simply confirms the
//               callback, for every band that isn't a pure inspection/emergency
//               case. The raw price is never shown in the UI, under any
//               circumstance — only ever communicated by a human callback. */}
//           <div className="border border-[#161616]/10 bg-white p-6 shadow-[0_2px_24px_rgba(22,22,22,0.05)]">
//             <p className="text-xs uppercase tracking-[0.3em] text-[#161616]/35 mb-4">
//               {score <= 25 || isEmergenza ? 'Stima intervento' : 'Prossimi passi'}
//             </p>
//             {score <= 25 ? (
//               <div>
//                 <p className="font-display text-xl text-[#161616]">Solo ispezione annuale</p>
...
//                 <CheckCircle2 className="size-5 text-[#EB1C26] shrink-0 mt-0.5" />
//                 <div>
//                   <p className="font-display text-lg text-[#161616] leading-tight">Richiesta inviata</p>
//                   <p className="text-sm text-[#494949] mt-1.5 leading-relaxed">
//                     Ti contattiamo entro 24h al numero indicato con il prezzo più competitivo in Italia per il tuo
//                     profilo, senza impegno.
//                   </p>
//                 </div>
//               </div>
//             )}
...
//   const [numberInput, setNumberInput] = useState('')
//   const [numberError, setNumberError] = useState('')
//   const [contactCitta, setContactCitta] = useState('')
//   const [contactTelefono, setContactTelefono] = useState('')
//   const [contactError, setContactError] = useState('')
//   const [result, setResult] = useState<CalcoloOutput | null>(null)
//   // Mirrors `answers` at the exact moment the quiz completes. Not currently
//   // read elsewhere, but kept alongside `result` so a stable, fully-typed
//   // CalcoloInput is available if the result panel ever needs it again.
//   const [finalInput, setFinalInput] = useState<CalcoloInput | null>(null)
...
//   // Final step: città (optional) + telefono (required). This is always the
//   // last step of the quiz, so it builds and finalizes the input directly
//   // instead of calling goNext() — the same reason selectChoice above does
//   // the same thing rather than reusing goNext(), since reading the
//   // `answers` closure here would miss this step's own answer otherwise.
//   const handleContactNext = useCallback(() => {
//     const parsedPhone = italianPhoneSchema.safeParse(contactTelefono)
//     if (!parsedPhone.success) {
//       setContactError(parsedPhone.error.issues[0]?.message ?? 'Numero di telefono non valido.')
//       return
//     }
//     setContactError('')
//     const citta = contactCitta.trim() || undefined
//     const telefono = parsedPhone.data
//     const input = { ...answers, citta, telefono } as CalcoloInput
//     setAnswers(prev => ({ ...prev, citta, telefono }))
//     setResult(calcolaRischio(input))
//     setFinalInput(input)
//     incrementAnalyzedCount()
//     notifyAnalysisCompleted(input)
//     trackCTAClick('roof_index_contact_submit')
//   }, [contactCitta, contactTelefono, answers])
...
//                   {/* Contact step (final step): città (optional) + telefono
//                       (required). Marketing wants the phone number captured
//                       here — inside the funnel, before the result — rather
//                       than as a post-result "unlock" ask, to maximize lead
//                       volume. The value banner leads with why we need it. */}
//                   {step.type === 'contact' && (
//                     <motion.div
//                       initial={{ opacity: 0, y: 20 }}
//                       animate={{ opacity: 1, y: 0 }}
...
//                     >
//                       <div className="flex items-start gap-3 border border-[#EB1C26]/20 bg-[#EB1C26]/[0.04] px-5 py-4">
//                         <Lock className="size-4 text-[#EB1C26] shrink-0 mt-0.5" />
//                         <p className="text-sm text-[#161616]/80 leading-relaxed">
//                           Abbiamo calcolato <strong>il prezzo più competitivo in Italia</strong> per il tuo profilo.
//                           Lascia il tuo numero: un nostro tecnico ti richiama entro 24h con l&apos;offerta
//                           personalizzata.
//                         </p>
//                       </div>

//                       <div>
//                         <label htmlFor="quiz-citta" className="text-xs font-bold uppercase tracking-widest text-[#161616]/40 mb-2 block">
//                           Città <span className="font-normal text-[#161616]/30">(facoltativo)</span>
//                         </label>
//                         <div className="relative flex items-center border border-[#161616]/15 bg-white focus-within:border-[#EB1C26] transition-colors">
//                           <input
//                             id="quiz-citta"
...
//                             placeholder="Es. Venezia, Chioggia, Roma..."
//                             value={contactCitta}
//                             onChange={e => setContactCitta(e.target.value)}
//                             onKeyDown={e => {
//                               if (e.key === 'Enter' && !e.nativeEvent.isComposing) {
//                                 e.preventDefault()
//                                 handleContactNext()
//                               }
//                             }}
//                             className="flex-1 bg-transparent px-5 py-4 text-lg font-display text-[#161616] placeholder:text-[#161616]/25 outline-none"
//                           />
//                         </div>
//                       </div>

//                       <div>
//                         <label htmlFor="quiz-telefono" className="text-xs font-bold uppercase tracking-widest text-[#161616]/40 mb-2 block">
//                           Numero di telefono <span className="text-[#EB1C26]">*</span>
//                         </label>
//                         <div className="relative flex items-center border border-[#161616]/15 bg-white focus-within:border-[#EB1C26] transition-colors">
//                           <input
//                             id="quiz-telefono"
...
//                           />
//                         </div>
//                         {contactError && <p className="text-xs text-[#EB1C26] mt-2">{contactError}</p>}
//                       </div>

//                       <button
//                         onClick={handleContactNext}
//                         className="flex items-center justify-center gap-2 bg-[#EB1C26] px-6 py-4 text-sm font-bold uppercase tracking-wider text-white shadow-[0_8px_24px_rgba(235,28,38,0.2)] hover:bg-[#161616] transition-colors self-start"
//                       >
//                         Ricevi la mia offerta migliore
//                         <ArrowRight className="size-4" />
//                       </button>

//                       <p className="text-[11px] text-[#161616]/35 leading-relaxed">
//                         Nessun impegno · Ti richiamiamo entro 24h · I tuoi dati non vengono mai condivisi con terzi
//                       </p>
//                     </motion.div>
//                   )}
//                 </motion.div>
...
interface RoofCalculatorProps {
  /** Hide the "← Tetto94" back-link in the top bar — it's redundant when
   *  this component is embedded directly on the homepage itself rather
   *  than on its own dedicated /calcola-preventivo page. Defaults to true
   *  (shown) to preserve existing behavior on that page. */
  showBackLink?: boolean
}

export default function RoofCalculator({ showBackLink = true }: RoofCalculatorProps) {
  const [currentStep, setCurrentStep] = useState(0)
  const [direction, setDirection] = useState(1)
  const [answers, setAnswers] = useState<Partial<CalcoloInput>>({})
  const [numberInput, setNumberInput] = useState('')
