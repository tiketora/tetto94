...
//   // Writes the number straight to the DOM on every tick instead of calling
//   // setState (which would re-render React ~60x/sec for each counter). With
//   // 3 counters animating at once on mount, the setState version was the
//   // main cause of the "not smooth" entrance jank reported on mobile.
//   useEffect(() => {
...
//     })
//     return () => controls.stop()
//   }, [inView, value, duration, motionValue, suffix])
...
// import { ShieldCheck, Camera, FileCheck2, ArrowDown, ArrowRight } from 'lucide-react'
// import { trackCTAClick } from '@/lib/gtag'
// import { BASE_ANALYZED_COUNT, ANALYSES_UPDATED_EVENT, getAnalyzedCount } from '@/lib/roof-index-stats'

// /* ─────────────────────────────────────────────────────────────────────────
//    Live "Tetti Analizzati" count — starts at the fixed baseline (so SSR and
//    first paint match), then syncs to this browser's real localStorage tally
//    on mount, and bumps up instantly whenever a quiz completes anywhere on
//    the page (see lib/roof-index-stats.ts).
//    ───────────────────────────────────────────────────────────────────────── */
// function useAnalyzedCount() {
//   const [count, setCount] = useState(BASE_ANALYZED_COUNT)

//   useEffect(() => {
//     setCount(getAnalyzedCount())
//     const handleUpdate = (e: Event) => {
//       const detail = (e as CustomEvent<number>).detail
//       if (typeof detail === 'number') setCount(detail)
//     }
//     window.addEventListener(ANALYSES_UPDATED_EVENT, handleUpdate)
//     return () => window.removeEventListener(ANALYSES_UPDATED_EVENT, handleUpdate)
//   }, [])
...
//   const [calculatorReached, setCalculatorReached] = useState(false)
//   const showSticky = !heroInView && !calculatorReached
//   const analyzedCount = useAnalyzedCount()
...
//           <div className="flex flex-col items-center">
//             <span className="font-display text-3xl text-[#161616] lg:text-4xl">
//               <CountUp value={analyzedCount} suffix="+" />
//             </span>
//             <span className="mt-1 text-[11px] font-bold uppercase tracking-[0.2em] text-[#161616]/40">
//               Tetti Analizzati
//             </span>
...
function useAnalyzedCount() {
  ...
  useEffect(() => {
    ...
    return () => {
      cancelled = true
      window.removeEventListener(ANALYSES_UPDATED_EVENT, handleUpdate)
    }
  }, [])
...
export default function RoofIndexIntro({ targetId }: { targetId: string }) {
  ...
  return (
    <div ref={rootRef} className="relative bg-white">
      ...
      >
        {/* subtle red glow, anchored (not decorative-only: reinforces the brand accent behind the kinetic headline).
            Uses a radial-gradient instead of a CSS `blur()` filter: a 36rem circle with a 120px blur sits inside
            a parent that's continuously transformed on scroll (heroY/heroOpacity below), and rasterizing+recompositing
            a filter:blur layer under transform is exactly the kind of work mobile GPUs choke on — this was the main
            source of the load/scroll jank reported on real phones (desktop GPUs hide it easily). A radial-gradient
            renders the same soft glow as a plain paint, with none of that cost. */}
        <div
          aria-hidden
          className="pointer-events-none absolute left-1/2 top-1/3 h-[36rem] w-[36rem] -translate-x-1/2 -translate-y-1/2 rounded-full"
          style={{
            background: 'radial-gradient(circle, rgba(235,28,38,0.09) 0%, rgba(235,28,38,0.03) 45%, transparent 72%)',
          }}
        />
        {/* faint architectural grid, grounds the light background so it doesn't feel empty */}
        <div
