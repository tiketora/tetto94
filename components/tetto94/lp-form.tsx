...
interface Props {
  ...
  formId: string  // unique per page instance (top/bottom)
  pageId: string  // e.g. 'lp_veneto' — sent to GA4/Ads for attribution
  service?: string  // e.g. 'Coibentazione Tetto' — defaults to the rifacimento LPs' original value
}

export default function LPForm({ region, formId, pageId, service = 'Rifacimento Tetto' }: Props) {
  const [status, setStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle')
  const [errorMsg, setErrorMsg] = useState('')
  const [form, setForm] = useState<LPFormData>({
    nome_cognome: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success')
      trackFormSubmit({ service: `${service} — ${region}`, city: form.citta, page_id: pageId })
    } catch {
      setErrorMsg('Errore di rete. Controlla la connessione e riprova.')
      setStatus('error')
    }
