...
export default function CookieBanner() {
  ...
  return (
    <>
      {/*
        Step 1: Set consent DEFAULTS — must run before any config call.
        `beforeInteractive` is reserved for scripts rendered directly in
        app/layout.tsx; nested inside this client component it gets
        rendered as a plain <script> tag instead of being hoisted (hence
        the "Encountered a script tag" warning). This component itself
        only mounts client-side after hydration, so `afterInteractive`
        is functionally equivalent here and keeps ordering: Next.js runs
        Script tags with the same strategy in the order they appear.
      */}
      <Script
        id="google-consent-default"
        strategy="afterInteractive"
