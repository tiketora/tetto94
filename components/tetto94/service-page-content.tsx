...
interface Props {
  ...
  location?: LocationConfig
  citySeoData?: CitySeoData
  /** Lightweight per-city unique paragraph for services without a full
   *  CitySeoData package (e.g. coibentazione-tetto) — takes priority over
   *  the generic "{longDescription} Operiamo a {city}..." fallback. */
  cityParagraph?: string
}
...
export default function ServicePageContent({ service, location, citySeoData, cityParagraph }: Props) {
  const ref = useRef(null)
  const inView = useInView(ref, { once: true, margin: '-6%' })
  const cityLabel = location ? ` a ${location.name}` : ' nel Nord-Est Italia'
  ...
  return (
    <div ref={ref} className="bg-white">
        ...
        <section className="bg-[#111] py-14 lg:py-18 border-t border-white/5">
          <div className="mx-auto max-w-7xl px-6">
            <motion.div variants={fadeUp(0)} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="mb-8">
              <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#EB1C26]">Prezzi</span>
              <h2 className="mt-2 font-display text-[clamp(1.8rem,3.5vw,2.8rem)] leading-none text-white">
                {service.costHeading ? service.costHeading.white : 'QUANTO COSTA IL'}{' '}
                <span className="text-[#EB1C26]">
                  {service.costHeading ? service.costHeading.red : `${service.name.toUpperCase()}?`}
                </span>
              </h2>
              <p className="mt-3 text-sm text-white/50 max-w-xl leading-relaxed">
                {service.costIntro ?? 'Prezzi indicativi per il Veneto, IVA inclusa. Il preventivo definitivo è gratuito e include sopralluogo drone entro 24 ore.'}
              </p>
            </motion.div>
      ...
      {/* ── 1b. Extra sections — e.g. "I 3 metodi di coibentazione", materiali, detrazioni ── */}
      {service.extraSections?.map((section, sIdx) => (
        <section
          key={section.kicker}
          className={sIdx % 2 === 0 ? 'bg-[#f5f5f5] py-16 lg:py-20 border-t border-black/5' : 'bg-white py-16 lg:py-20'}
        >
          <div className="mx-auto max-w-7xl px-6">
            <motion.div variants={fadeUp(0)} initial="hidden" animate={inView ? 'visible' : 'hidden'} className="mb-10 max-w-3xl">
              <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#EB1C26]">{section.kicker}</span>
              <h2 className="mt-2 font-display text-[clamp(1.8rem,3.5vw,2.8rem)] leading-none text-[#161616]">
                {section.heading.white} <span className="text-[#EB1C26]">{section.heading.red}</span>
              </h2>
              {section.intro && (
                <p className="mt-3 text-sm text-[#494949] leading-relaxed">{section.intro}</p>
              )}
            </motion.div>

            {section.blocks && (
              <div className={`grid gap-4 ${section.blocks.length === 3 ? 'sm:grid-cols-3' : 'sm:grid-cols-2'}`}>
                {section.blocks.map((block, i) => (
                  <motion.div
                    ...
                    initial="hidden"
                    animate={inView ? 'visible' : 'hidden'}
                    className="border border-black/8 bg-white p-6"
                  >
                    <div className="w-8 h-0.5 bg-[#EB1C26] mb-4" />
                    <h3 className="font-sans font-bold text-sm text-[#161616] leading-snug mb-2">{block.title}</h3>
                    <p className="text-xs text-[#494949] leading-relaxed">{block.body}</p>
                  </motion.div>
                ))}
              </div>
            )}
            ...
            {section.body && (
              <motion.p
                ...
                initial="hidden"
                animate={inView ? 'visible' : 'hidden'}
                className="text-sm text-[#333] leading-relaxed max-w-3xl"
              >
                {section.body}
              </motion.p>
            )}
      ...
      <section className="bg-[#EB1C26] py-14">
        <div className="mx-auto max-w-7xl px-6 flex flex-col lg:flex-row items-center justify-between gap-6">
          <div>
            <p className="font-display text-[clamp(1.6rem,3vw,2.4rem)] text-white leading-tight">
              {service.ctaHeading
                ? <>{service.ctaHeading.white} <span className="opacity-90">{service.ctaHeading.red}</span></>
                : <>PRONTO PER IL TUO {service.name.toUpperCase()}{location ? ` A ${location.name.toUpperCase()}` : ''}?</>
              }
            </p>
            <p className="mt-1 text-sm text-white/80">
