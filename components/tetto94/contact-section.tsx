...
export default function ContactSection() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success')
      trackFormSubmit({ service: form.servizio, city: form.citta })
    } catch {
  ...
  const inputClass =
    'w-full rounded-sm border border-black/10 bg-white px-4 py-3 text-sm text-[#161616] placeholder-[#999] focus:border-[#EB1C26] focus:outline-none focus:ring-1 focus:ring-[#EB1C26] transition-colors'

  return (
    <section id="contatti" className="bg-white py-24 lg:py-32 border-t border-black/5" ref={ref}>
      <div className="mx-auto max-w-7xl px-6">
        ...
        >
          ...
          <p className="mt-4 text-base text-[#333] max-w-2xl mx-auto leading-relaxed">
            Richiedi un&apos;ispezione gratuita del tuo tetto con drone.<br className="hidden sm:block" />
            Il nostro team specializzato individua rapidamente eventuali danni o problemi e ti offre una soluzione professionale per la riparazione del tetto.
          </p>
          <p className="mt-3 text-sm font-semibold text-[#EB1C26]">
            Tutti i nostri interventi sono coperti da 10 anni di garanzia.
          </p>
          <p className="mt-3 text-sm text-[#494949] max-w-xl mx-auto">
        ...
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          >
            {/* Info cards */}
            {[
              { icon: Phone, label: 'Telefono', value: '+39 351 651 9363', href: 'tel:+393516519363', track: true as const },
              { icon: Mail, label: 'Email', value: 'info@tetto94.it', href: 'mailto:info@tetto94.it', track: false },
              { icon: MapPin, label: 'Indirizzo', value: 'Via Benedetto Veruda, 30100 Venezia VE — Operiamo in tutto il Nord-Est Italia', href: 'https://maps.app.goo.gl/dzGd69uJaH4x34W9A', track: false },
              { icon: Clock, label: 'Orari', value: 'Lun – Sab: 8:00 – 18:00', href: undefined, track: false },
            ].map((item) => {
              const Icon = item.icon
              const content = (
                <div className="flex items-start gap-4 rounded-sm border border-black/10 bg-[#f9f9f9] p-5 hover:border-[#EB1C26]/40 transition-colors">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-sm font-semibold text-[#161616]">{item.value}</p>
                  </div>
                </div>
              )
              return item.href ? (
                <a
                  key={item.label}
                  href={item.track ? undefined : item.href}
                  onClick={item.track ? (e) => { e.preventDefault(); trackPhoneClick('contact_section') } : undefined}
                >
                  {content}
                </a>
              ) : (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-[#494949]">Risposta rapida garantita</p>
              </div>
            </a>
          </motion.div>
        </div>
